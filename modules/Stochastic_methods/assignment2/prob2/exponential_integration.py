...
def monte_carlo_integration(funct_to_average, N):
    ...
    sum_of_rands = 0
    for k in rand_nrs:
        sum_of_rands += funct_to_average(-(math.log(1-k))/3) #exp distr!!
    ...
    for k in rand_nrs:
        sum_of_rands_approx += (funct_to_average(-(math.log(1-k))/3)-average_result)**2  #exp distr!!
...
print monte_carlo_integration(funct1, 1000000)
