...
//Tags for communication between processes:
//  -- 1: MPI_INT (1 or 0), indicating if the setup data is appropriate.. if not, abort
//  -- 2: associated to the arrays of memory for which the fitnesses will be calculated
//  -- 3: fitness values sent from Slaves to ROOT
...
int main(int argc, char **argv){
  ...
  //Common code to all processes
  int my_id, root_process, num_procs;
  int buff_comm; //integer used for communicating between ROOT and the Slaves
 ...
 root_process = 0;
  ...
  //Distribution of code between processes
  if(my_id != root_process){
  
    MPI_Recv(&buff_comm, 1, MPI_INT, root_process, 1, MPI_COMM_WORLD, &status);
    
    if(buff_comm == 0){
      printf("..process %d aborting because of ROOT instructions.\n", my_id);
      MPI_Finalize();
      return 0;
    }
    else{
      ...
      int i, j, k;
      int chrom_size = 16, pop_s, nr_gens, indiv_size, iterns_pd;
      int buff_int;
      int *partial_info;
    
      //need here the population size
      for(i=1; i<argc; i++){
        if(strcmp(argv[i], "-ps") == 0){
          pop_s = atoi(argv[i+1]);
        }
        else if(strcmp(argv[i], "-ng") == 0){
          nr_gens = atoi(argv[i+1]);
        }
      ...
      //calculating some information needed to store the data sent from ROOT
      indiv_size = (chrom_size-chrom_size%(8*sizeof(int)))/(8*sizeof(int));
      if(chrom_size%(8*sizeof(int)) != 0){indiv_size++;}
      
      partial_info = (int *)malloc((pop_s*indiv_size)*sizeof(int));
      
      //array for storing fitness values
      int *fitness_values = (int *)malloc((pop_s/num_procs)*sizeof(int));
      
      //population_size*indiv_size/num_procs
      for(i=0; i<nr_gens; i++){
      
        //for each generation, receive updated data
        MPI_Recv(partial_info, pop_s*indiv_size, MPI_INT, root_process, 2, MPI_COMM_WORLD, &status);
        
        //calculate the fitness and store it in an array of floats
        for(j=0; j<pop_s/num_procs; j++){
          //fitness:
          buff_int = 0;
          for(k=0; k<indiv_size; k++){
            if(k==0){
              buff_int += fitness_fnctn(indiv_size, indiv_size*(8*sizeof(int))-chrom_size,
			iterns_pd, pop_s, partial_info, (pop_s*indiv_size/num_procs)*my_id+j*indiv_size+k);
            }
            else{buff_int += fitness_fnctn(indiv_size, 0, iterns_pd, pop_s, partial_info,
            		(pop_s*indiv_size/num_procs)*my_id+j*indiv_size+k);}
          }
          fitness_values[j] = buff_int;
        }
        
        //send to ROOT values of fitnesses for each iteration
        MPI_Send(fitness_values, pop_s/num_procs, MPI_INT, root_process, 3, MPI_COMM_WORLD);
      }
    }
  }

  else{
    /* BEGINNING OF ROOT'S CODE */
    ...
    int indx; //variables used exclusively for sending (receiving) to (from)
                         //processes, within ROOT
    ...
    if((argc-1)%2 != 0){
      printf("ROOT aborting: wrong number of parameters.\n\n");
      buff_comm = 0;
      for(indx = 1; indx < num_procs; indx++){
        MPI_Send(&buff_comm, 1, MPI_INT, indx, 1, MPI_COMM_WORLD);
      }
      MPI_Finalize();
      return 0;
    }
    ...
    for(i=1; i<argc; i++){
      ...
      //}
        ...
        if(i%2 != 0){
          printf("ROOT aborting: wrong setup numbers.\n\n");
          buff_comm = 0;
          for(indx = 1; indx < num_procs; indx++){
            MPI_Send(&buff_comm, 1, MPI_INT, indx, 1, MPI_COMM_WORLD);
          }
    ...
    if(i != 11){
      printf("ROOT aborting: wrong format in data.\n\n");
      buff_comm = 0;
      for(indx = 1; indx < num_procs; indx++){
        MPI_Send(&buff_comm, 1, MPI_INT, indx, 1, MPI_COMM_WORLD);
      }
    ...
    if((buff_doub-(int)buff_doub) != 0){
      printf("ROOT aborting: input error: make (ps*cr/100)/2 be an integer.\n\n");
      buff_comm = 0;
      for(indx = 1; indx < num_procs; indx++){
        MPI_Send(&buff_comm, 1, MPI_INT, indx, 1, MPI_COMM_WORLD);
      }
    ...
    if((buff_doub-(int)buff_doub) != 0){
      printf("ROOT aborting: input error: make ps*mr/100 be an integer.\n\n");
      buff_comm = 0;
      for(indx = 1; indx < num_procs; indx++){
        MPI_Send(&buff_comm, 1, MPI_INT, indx, 1, MPI_COMM_WORLD);
      }
    ...
    //Check if the number of processes divides the size of total_info
    if((i*population_size)%num_procs != 0 || population_size%num_procs != 0){
      printf("ROOT aborting: input error: inappropriate nr of processes.\n\n");
      buff_comm = 0;
      for(indx = 1; indx < num_procs; indx++){
        MPI_Send(&buff_comm, 1, MPI_INT, indx, 1, MPI_COMM_WORLD);
      }
    ...
    //But if all good with data so far, then send positive signal to Slaves
    buff_comm = 1;
    for(indx = 1; indx < num_procs; indx++){
      MPI_Send(&buff_comm, 1, MPI_INT, indx, 1, MPI_COMM_WORLD);
    }
    ...
    for(i=0; i<nr_generations; i++){
      ...
      //send, to all Slaves, each corresponding memory chunk.. updated data
      for(indx = 1; indx<num_procs; indx++){
        MPI_Send(total_info, population_size*indiv_size, MPI_INT, indx, 2, MPI_COMM_WORLD);
      }
    
      //as this is ROOT, calculate fitnesses for just it's corresponding data
      //calculate the fitness and store it in an array of floats
      for(j=0; j<population_size/num_procs; j++){
        //fitness:
        buff_int = 0;
        for(k=0; k<indiv_size; k++){
          if(k==0){
      ...
      //gather fitnesses sent back from all the Slaves. This is the step that might
      //mess with the total Fitness values
      for(indx = 1; indx<num_procs; indx++){
        MPI_Recv(fitness_values+(population_size/num_procs)*indx, population_size/num_procs,
        	MPI_INT, indx, 3, MPI_COMM_WORLD, &status);
      }
